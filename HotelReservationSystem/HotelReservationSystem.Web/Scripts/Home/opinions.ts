            ...
            opinions.forEach((op, i) => {
                ...
                if (carouselIndicators) {
                    ...
                    carouselIndicators.insertAdjacentHTML('beforeend', 
                        `<button type="button" data-bs-target="#opinionsCarousel" data-bs-slide-to="${i}" ${activeClass} aria-label="Slide ${i + 1}" style="background-color: #23405c;"></button>`
                    );
                }
                ...
                const guestName = op.guestFirstName ? `${op.guestFirstName} ${op.guestLastName ?? ''}` : 'Anonim';
                    ...
                    <div class="carousel-item ${activeItemClass}">
                        <div class="review-item card h-100 border-0 shadow-sm rounded-3">
                            ...
                            <div class="card-body bg-light text-start p-4">
                                <p class="review-text m-0 text-dark">${op.comment ?? ''}</p>
                            </div>
                        </div>
                    </div>`;
